...
def main() -> int:
    ...
    run_step("Download data", [py, "-m", "rf_pipeline.download_data", "--ticker", ticker], cwd=src_root)

    build_cmd = [py, "-m", "rf_pipeline.build_dataset", "--ticker", ticker]
    train_cmd = [py, "-m", "rf_pipeline.train_rf", "--ticker", ticker]
    eval_cmd = [py, "-m", "rf_pipeline.evaluate", "--ticker", ticker]
    if args.start_date:
